...
from g7_gates import evaluate_ask_band, ask_band_config
...
ALLOCATED_USD = float(os.environ.get("ALLOCATED_USD", "1.0"))
...
def evaluate_v2(card: dict, trend: str = "NEUTRAL") -> dict:
    ...
    # G7 ask-band (ASK_MAX/ASK_MIN); shadow unless GATE_SHADOW=0
    g7info = evaluate_ask_band(ask)
    g7 = g7info["g7"]
    g7_configured = g7info["g7_configured"]
    g7_would_pass = g7info["g7_would_pass"]
    ask_max = g7info["ask_max"]
    ask_min = g7info["ask_min"]
    gate_shadow = g7info["gate_shadow"]
    hard_g7 = g7info["hard_g7"]

    execute = bool(g1 and g2 and g3 and price_ok and g5 and g4 and g6 and hard_g7)
    ...
    if g7_configured and (not gate_shadow) and (not g7_would_pass):
        extra_reasons.append(
            f"G7 ask-band failed: ask={ask} not in [{ask_min}, {ask_max}] (GATE_SHADOW=0)."
        )
    ...
    return {
        ...
        "g6": g6,
        "g7": g7,
        "g7_configured": g7_configured,
        "g7_would_pass": g7_would_pass,
        "ask_max": ask_max,
        "ask_min": ask_min,
...
def run_one_cycle(bot: PolymarketAgentBot) -> dict:
    ...
    logger.info(
        "Gates G1=%s G2=%s G3=%s G4(quality)=%s G5(trend)=%s G6(regime)=%s price_ok=%s "
        "G7(ask-band)=%s configured=%s would_pass=%s shadow=%s | side=%s p=%.4f ask=%s edge=%.4f conf=%s",
        ev["g1"],
        ev["g2"],
        ev["g3"],
        ev["g4"],
        ...
        ev["price_ok"],
        ev.get("g7"),
        ev.get("g7_configured"),
        ev.get("g7_would_pass"),
        ev.get("gate_shadow"),
        ev.get("side"),
...
def main():
    ask_max, ask_min, gate_shadow, g7_configured = ask_band_config()
    logger.info(
        "Starting balanced-v2 cycle runner (EDGE_BUFFER=%.2f, size=$%.2f, MIN_BALANCE=$%.2f) "
        "G7 ASK_MAX=%s ASK_MIN=%s GATE_SHADOW=%s configured=%s",
        EDGE_BUFFER, ALLOCATED_USD, MIN_BALANCE, ask_max, ask_min, int(gate_shadow), g7_configured,
    )
    try:
        init_db()
